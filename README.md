...
*   **导出优化**：
    *   导出图片格式从 PNG 改为 JPG，文件大小减少 60-70%
    *   导出图片添加模糊背景效果，提升视觉质感
    *   导出卡片重新布局：图片置顶（最高 300px）、标题简洁、版本号移至底部
    *   优化导出按钮状态显示"导出中..."，防止重复点击
*   **图片管理**：
    *   修复图片 URL 失败后无法替换的问题
    *   图片加载失败时显示 300×300 占位符
    *   支持加载失败后重新上传图片或设置 URL
*   **移动端优化**：
    *   优化手机端顶部标题和按钮布局，减少拥挤感
    *   移动端隐藏版本号和刷新按钮，优化图标尺寸
    *   改进移动端图片导出体验，支持 Web Share API 保存到相册
*   **导入体验提升**：
    *   移除 `alert()` 和 `window.confirm()`，使用友好的 Toast 提示和自定义对话框
    *   添加导入加载状态和完整反馈流程
    *   完整备份导入时显示详细数据统计信息
    *   修复手机端导入模板"选择文件后没有动静"的问题
*   **导出内容优化**：
    *   底部水印区域增加版本号（动态获取）和网址二维码
    *   移除 Prompt Template 标签，简化底部信息展示
